<?php
	...
	$data = json_decode(file_get_contents("../strings.json"), true);
	...
	$mail->Subject = "Contato através do site <b>{$site_name}</b>.";

	$form_mail = @$data->contact->form_mail ? $data->contact->form_mail : $data->website["contact-mail"];
	foreach( $data->contact->form_mail as $recipient ){
	...
	$field = @$data->contact->form->fields ? $data->contact->form->fields : $data->pages->contact->form->fields;
	$textarea_title = @$data->contact->form->textarea->title ? $data->contact->form->textarea->title : $data->pages->contact->form["message-field]->name;
	foreach( $data->contact->form->fields as $key => $field ){
		if( @$_POST["{$field->name}"] ){
